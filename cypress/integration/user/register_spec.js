describe('Register', () => {
    beforeEach(() => {
        cy.visit('http://localhost:8888/webflix/register.php')
...
it('requires valid date of birth', () => {
    cy.get('#reg_form').within(() => {
...
it('requires input security answer', () => {
    cy.get('#reg_form').within(() => {
        cy.get(':nth-child(9) > .form-control').then($el => $el[0].checkValidity()).should('be.false')
    })
...
it('requires passwords match', () => {
    ...
    cy.get(':nth-child(4) > .form-control').type('1999-01-11')
    cy.get(':nth-child(5) > .form-control').invoke('val', '0123456789')
    cy.get('#inputGroupSelect01').select('United Kingdom')
    cy.get(':nth-child(7) > .form-control').invoke('val', '1')
    cy.get(':nth-child(8) > .form-control').invoke('val', '2')
    cy.get(':nth-child(9) > .form-control').invoke('val', 'Question')
    cy.get(':nth-child(10) > .form-control').invoke('val', 'Answer')
    cy.get('.btn').click()
...
it('redirects to choose subscription on successful registration', () => {
    cy.get('[name="first_name"]').invoke('val', 'Cypress')
    cy.get('[name="last_name"]').invoke('val', 'Testing')
    cy.get(':nth-child(3) > .form-control').invoke('val', 'exampleEmail@cypress.com')
    cy.get(':nth-child(4) > .form-control').type('1999-01-11')
    ...
    cy.get('#inputGroupSelect01').select('United Kingdom')
    cy.get(':nth-child(7) > .form-control').invoke('val', '1')
    cy.get(':nth-child(8) > .form-control').invoke('val', '1')
    cy.get(':nth-child(9) > .form-control').invoke('val', 'Question')
    cy.get(':nth-child(10) > .form-control').invoke('val', 'Answer')
    cy.get('.btn').click()

    cy.get('h1')
    .should('have.text', "Choose a subscription option")
