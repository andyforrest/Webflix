...
# Check if email address already registered.
  if ( empty( $errors ) )
  {
    $q = "SELECT * FROM admins WHERE email='$e'" ;
    $r = @mysqli_query ( $link, $q ) ;
    }
# On success new password into 'admins' database table.
  if ( empty( $errors ) ) 
  {
    $q = "UPDATE admins SET pass= SHA2('$p',256) WHERE admin_id={$_SESSION[ 'admin_id' ]}";
    $r = @mysqli_query ( $link, $q ) ;
    if ($r)
    {
