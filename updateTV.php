...
# Check to see if form filled in.
  ...
  if ( $_POST['type'] == "TV Show" && empty( $_POST[ 'num_seasons' ] ) )
  { $errors[] = 'Number of TV Show Seasons cannot be blank' ; }
  else
  { $num_seasons = mysqli_real_escape_string( $link, trim( $_POST[ 'num_seasons' ] ) ) ; }

  if ( $_POST['type'] == "TV Show" && empty( $_POST[ 'num_episodes' ] ) )
  { $errors[] = 'Number of TV Show Episodes cannot be blank' ; }
  else
  { $num_episodes = mysqli_real_escape_string( $link, trim( $_POST[ 'num_episodes' ] ) ) ; }

 if(empty($errors)){
    
     $sql = "UPDATE media SET media_title = '$mt', category = '$cat', further_info = '$fi', img = '$img', type = '$ty', preview = '$pv', release_year = '$release_year', language = '$language', duration = '$duration', 
     num_seasons = '$num_seasons', num_episodes = '$num_episodes' WHERE media_id = {$_SESSION['id']}";
